...
impl App {
    ...
    fn event(&mut self, event: Event) -> Effect {
        ...
        if (!text_editing || command_key(key)) && self.bindings.direct(key, Action::Pane) {
            self.toggle_panes();
            return Effect::None;
        }
        if self.pane_mode {
        ...
        if text_editing {
            if let Some(action) = self.bindings.insert_motion(key) {
                return self.input.action(action).into();
            }
    ...
    fn dialog_event(&mut self, key: KeyEvent) -> Effect {
        ...
        if d.field == 0 {
            if d.text.get_mode() != InputMode::Insert {
                let context = if d.text.get_mode() == InputMode::Visual {
                    Context::Visual
                } else {
                    Context::Input
                };
                        ...
                        .feed(&self.bindings, key, context, Instant::now())
                {
                    match action {
                        ...
                        _ => return d.text.action(action).into(),
                    }
                }
                return Effect::None;
            }
            if !(command_key(key) && self.bindings.direct(key, Action::Submit)) {
                if let Some(action) = self.bindings.insert_motion(key) {
                    return d.text.action(action).into();
                }
                d.text.edit(key);
                return Effect::None;
            }

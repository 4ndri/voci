...
The workflow fetches full Git history and calculates GitVersion's `SemVer` before building. Every platform verifies that its calculated version matches that shared version. Publishing creates the corresponding `v<SemVer>` tag at the exact commit that triggered the workflow, even if `main` has advanced in the meantime. Existing release tags remain part of GitVersion's history for calculating subsequent versions. Use `mise run version` to preview the calculated version locally. Versions with a prerelease suffix are published as prereleases; local feature branches receive GitVersion's prerelease labels.
